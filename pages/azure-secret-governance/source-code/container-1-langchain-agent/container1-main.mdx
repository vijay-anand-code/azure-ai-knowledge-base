...
**What it does:** Hosts the LangChain agent (GPT-4o) that orchestrates the full governance cycle. Defines all tools that call Container 2, manages per-session chat memory, and exposes FastAPI endpoints for the runbook trigger, chat, the Jira status-update proxy, and (when enabled) the Teams bot.
...
| `/jira-status-update` | POST | Plain proxy to Container 2's `/tools/jira-status-update`, gives the Jira automation rule one stable externally-reachable URL, regardless of which container ingress currently points at |
| `/teams-webhook` | POST | Receives Teams Bot messages, routes through LangChain agent |

<Callout type="warning">
  The Teams bot integration (the `from teams_bot import ...` block near the bottom of the file)
  is currently **commented out** in the deployed source. The Teams bot has been removed from
  this deployment for now. `/teams-webhook` is still named in the module docstring and in the
  table above because that's what the docstring itself claims, but with the import/router lines
  commented out, `app.include_router(teams_router)` never runs, so this route does not actually
  exist on the live app today. See the [Teams bot](/azure-secret-governance/teams-bot) page for
  what's involved in reinstating it.
</Callout>

<Callout type="info">
  `/jira-webhook` was retired from this container: it only ever recognized 3 status names and
  did nothing for the rest. It has **not** been replaced by Jira calling Container 2 directly.
  Instead, Container 1 now exposes `/jira-status-update`, a thin proxy that forwards straight to
  Container 2's `/tools/jira-status-update`, added specifically because a Container App only
  exposes one external port at a time and that port has to stay on Container 1 for `/run` to keep
  working. See the [Jira Automation](/azure-secret-governance/jira) page for the full setup.
</Callout>
...
| Per-session `InMemoryChatMessageHistory` | Multi-turn conversations (chat, or Teams bot if re-enabled) maintain context across messages in the same session |
...
| `/jira-status-update` is a plain proxy | No status-mapping logic lives in Container 1: it just forwards the request body to Container 2's `/tools/jira-status-update` over `_call_monitor`, so there is a single source of truth for the status mapping |

<Callout type="info">
  The Teams bot is enabled by uncommenting 6 lines near the bottom of this file (the
  `from teams_bot import ...` block). Today those lines are commented out, so the bot is disabled
  and `/teams-webhook` does not exist on the running app, so re-enabling it is just uncommenting
  those lines and redeploying, no other code changes needed.
</Callout>
...
| `get_sharepoint_state` | `/tools/get_sharepoint_state` | Read the SharePoint master list |
...
<Callout type="warning">
  The current source's docstrings for `get_sharepoint_state` and `write_sharepoint_row` (and the
  `SYSTEM_PROMPT` string below) call the list `DevSecOpsSecretMonitoringRegistry`. That name is
  stale inside this file itself: the actual SharePoint list Container 2 reads and writes is
  `SecretAlertRegistry` (see the [SharePoint Setup](/azure-secret-governance/sharepoint) page).
  This doc mirrors the source as written; the mismatch is a leftover in the source's own comments,
  not something this page invented.
</Callout>
...
<Callout type="info">
  `BOT-APP-ID`, `BOT-APP-SECRET`, and `BOT-TENANT-ID` are only read if the Teams bot integration
  block is uncommented (see above); with it commented out, as it currently is, `_kv_get()` is
  never called for those three secrets and Container 1 only actually needs Key Vault access for
  the three Azure OpenAI secrets above. All Graph API, SharePoint, and Jira credentials are loaded
  separately by Container 2. The UAMI only needs `Key Vault Secrets User` role, no Graph
