...
| `/tools/update_on_jira_reopen` | POST | Legacy compatibility shim — calls `/tools/jira-status-update` with `to_status="In Progress"` |
...
| Jira webhook handled directly | Status transitions don't need AI reasoning — a direct SharePoint update is faster (2–3s vs 30–60s) and avoids OpenAI rate limits |
...
| `fetch_app_owners()` prefers display names | Returns a comma-separated list of Entra display names now (falling back to UPN/email only when Entra has no display name for that user) — an explicit client requirement ("names, not emails"). An earlier version always returned the UPN |
...
backup lists before every monitoring run touches either one — SharePoint has no built-in
...
| `backup_list_row_fields(source_fields)` | Strips SharePoint system/read-only fields (`id`, `Modified`, `_UIVersionString`, etc.) before a row gets re-created in a backup list — an allowlist-by-exclusion, so new business columns added to the source list later get backed up automatically with no code change |
...
on — the old `DevSecOpsOwnership` Jira custom field was removed from Jira entirely; ownership is
