...
| Date/time columns carry full EST timestamps, not truncated | `LastChecked`, `AlertSentDate`, `RotationDetectedDate`, `JiraTicketCreatedDate`, and `ExpirationDate` now arrive pre-formatted (via `decision_engine.py`'s `_format_datetime_est`/`_now_est_string`, matching `runbook_discovery.py`'s format exactly) rather than as raw ISO strings, so `write_sharepoint_row` no longer slices any of them to 10 characters. An earlier version truncated all five to `YYYY-MM-DD`, which is what made the time portion disappear whenever monitoring wrote to a row after discovery had already stamped a full timestamp on it |
