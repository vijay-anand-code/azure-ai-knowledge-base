...
|---|---|---|
| `move_secret_to_ignored()` | master list → Ignored (create in Ignored, then delete from master) | `decision_engine.py`'s -8-day abandonment check; `/tools/jira-status-update`'s Canceled handling |
...
| `move_secret_from_ignored()` | Ignored → master list (create in master, then delete from Ignored) | `/tools/jira-status-update`, when a ticket not found in the master list is found in `IgnoredSecretRegistry` instead, i.e. a Canceled ticket got reopened |
| `get_ignored_sharepoint_state()` | read-only | both of the above reverse-lookup paths |
...
The request body carries `jira_key`, `to_status`, `today`, and six custom fields: `team_name`,
`product_name`, `product_teams_key_vault_name`, `new_secret_vault_name`, `new_secret_key_id`, and
`new_secret_present`. A seventh field, `devsecops_ownership`, is still accepted on the request
model for backwards compatibility but is never sent by the current automation rule and never acted
on, since the old `DevSecOpsOwnership` Jira custom field was removed from Jira entirely; ownership is
resolved from `ProductName` via Key Vault instead.
...
- Also recognizes Done, Canceled/Cancelled, Blocked, and "Awaiting Reporter: Needs more
  ...
  rather than an in-place status change.
- Backfills `TeamName`, `ProductName`, `ProductTeamsKeyVaultName`, `NewSecretVaultName`, and
  `NewSecretKeyId` onto the matching row on both the In Progress and Resolved transitions, but
  only into columns that are still blank, never overwriting a value already set directly in
  SharePoint.
