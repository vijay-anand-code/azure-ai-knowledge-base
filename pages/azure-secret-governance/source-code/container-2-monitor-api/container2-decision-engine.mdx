...
| Bucket | Days | Alert status set | Jira priority | Teams alert |
|--------|------|-----------------|--------------|-------------|
| P1, 0 to 3 days | 0–3 | `CriticalTagged` | `P1` | Yes — CRITICAL, tags a specific person |
| P1, 4 to 7 days | 4–7 | `Escalated` | `P1` | Yes — CRITICAL, no tagging |
| P2 | 8–30 | `TeamsAlerted` | `P2` | Yes — WARNING |
| P3 | 31–60 | `JiraRaised` | `P3` | No |
| P4 | 61+ | Written to SharePoint, owners stamped, refreshed every run | none, never ticketed | No |
| ExpiredManualReview | -7 to -1 | `ExpiredManualReview` | `P1` | Yes — EXPIRED |
| Ignore, subject to abandonment check | -8 and older | `OverdueManualReview` if the Jira ticket is still open, otherwise moved to `IgnoredSecretRegistry` | none | No |
...
| `sp_index` keyed by `SecretID` alone, never gated by ownership | `SecretID` (Entra's `keyId`) is already a globally unique GUID, no need for a composite key. Indexing every existing row regardless of `ManualAppOwners` fixed a real duplication bug, see the callout below |
| `LastChecked`/`AlertSentDate`/`RotationDetectedDate`/`JiraTicketCreatedDate`/`ExpirationDate` as EST strings | All five now go through `_format_datetime_est()`/`_now_est_string()`, matching `runbook_discovery.py`'s format exactly, instead of a bare UTC date or a raw Entra ISO string |
...
<Callout type="warning">
  ...
  secret") only included rows where `ManualAppOwners` was already set, excluding any unowned
  row entirely. For P1-P3 that mistake was harmless, a genuinely new secret still can't get a
  row created without passing its own separate ownership check first. P4 has no equivalent check
  on its create path, so an app that only ever has P4 secrets, with nobody having filled in
  `ManualAppOwners` for it, had its existing P4 row invisible to `sp_index` every run, looked
  "new" every run, and got a duplicate row created every single run, forever. `sp_index` now
  includes every row unconditionally, ownership only affects the `skippedNotOwned` count, not
  whether a row is indexed.
</Callout>
...
async def run_secret_monitoring(
    ...
    # any row whose ManualAppOwners/AppOwners didn't pass the filter below,
    ...
    for item in sp_data.get("items", []):
        ...
        if manual_owners_only:
            if not (f.get("ManualAppOwners") or "").strip():
                skipped_not_owned += 1
