...
| Phase 3 — new P5 secrets | Log only — no SharePoint row, no alert |
...
|--------|------|-----------------|-------------|
| P1 | 0–3 | `CriticalTagged` | Yes — CRITICAL, tags a specific person |
| P2 | 4–7 | `Escalated` | Yes — CRITICAL, no tagging |
| P3 | 8–30 | `TeamsAlerted` | Yes — WARNING |
| P4 | 31–60 | `JiraRaised` | No |
| P5 | 61+ | Written to SharePoint if already tracked and refreshed each run; brand-new P5 secrets are log only | No |
...
| `ManualAppOwners` gate | Monitoring is gated on `ManualAppOwners` in SharePoint, filled in by a human, and propagated app-wide before filtering. Legacy `owner_emails` matching against `AppOwners` is still accepted as a parameter but ignored by default |
