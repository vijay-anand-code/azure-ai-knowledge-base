...
<Callout type="info">
  **Step 3's failure reporting is per-row, not first-error-only.** `_rotate_backup_list()`
  attempts every row in the snapshot regardless of earlier failures (`_run_batched()` runs every
  batch through to completion), so a write can fail on more than one row in the same run, each
  for a potentially different reason. `_create_one()` logs each failing row's `SecretID`/`AppName`
  immediately as it happens, and `create_list_row()` itself (in `main.py`) logs Graph's exact
  error body alongside that same row's `SecretID`/`AppName`/`AlertStatus` &mdash; so any future
  failure here is diagnosable straight from container logs, without needing to reproduce it or
  guess which row out of a large snapshot was responsible. The run summary's error message states
  an accurate failed-row count and names the first failure specifically, rather than the earlier
  wording that implied the write had stopped early (`_run_batched()` never stops early; that
  phrasing was corrected alongside this).
</Callout>
