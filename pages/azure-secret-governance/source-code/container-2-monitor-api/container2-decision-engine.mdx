...
**What it does:** Pure orchestration logic — no HTTP wiring. Receives injectable async functions from `main.py` and uses them to snapshot/back up SharePoint state, classify secrets, raise Jira tickets, send Teams alerts, and write SharePoint rows. All I/O happens in batches to stay within API rate limits.
...
The injection pattern means the engine has no knowledge of HTTP, Key Vault, or Azure — it can be unit-tested by passing in mock functions. The full parameter list is considerably longer than shown above (roughly thirty callables/values); see the signature in Source below.
...
| Phase 3 — new P4 secrets (61+ days, safe) | Written to SharePoint with `AlertStatus=Discovered`, no alert, no ticket. An earlier version of this phase only logged these to the console with no SharePoint row at all, inconsistent with how an already-existing P4 row was treated — fixed so both paths write the same way |
...
| Phase 4 — existing secrets | Escalate or update existing rows based on current bucket vs. recorded status, purely on severity — ownership is not a factor |
...
<Callout type="warning">
  ...
  message-building code exists but is unreachable through this path — don't assume an
...
<Callout type="info">
  ...
  `IgnoreReason: "Expired8Plus"` and no `JiraTicketKey` — there was never a ticket for it. An
...
ticket or alert — that's bucket-based, not ownership-based.
...
<Callout type="info">
  `ProductTeamkeyVaultName` (note the exact casing used by this file), a separate column, is
  unrelated to this lookup. It controls where `runbook_rotation.py` stores a newly rotated
  secret's value, not where `DevSecOpsOwnership` comes from.
</Callout>

<Callout type="warning">
  `main.py`'s `/tools/jira-status-update` handler backfills a differently-spelled column,
  `ProductTeamsKeyVaultName` (with an "s" after "Team"), from a Jira custom field. This file's own
  lineage tracking (below) reads and writes `ProductTeamkeyVaultName` (no "s"). These are two
  distinct SharePoint column names as the source currently stands — a value backfilled through one
  path does not automatically appear in the other's column. Confirm which spelling the live
  SharePoint list schema actually uses before assuming the two are kept in sync.
</Callout>
...
`TeamName`, `ProductName`, and `ProductTeamkeyVaultName` are filled in by hand, once, the first
...
row copies `TeamName`, `ProductName`, and `ProductTeamkeyVaultName` straight from the matching
...
<Callout type="info">
  ...
  `TeamName`/`ProductName`/`ProductTeamkeyVaultName` on any row, new or existing, so this lineage
...
3. Write a fresh snapshot of the current items into the backup list as new rows, batched the same
   ...
   run touched it) and the run aborts — nothing downstream runs this cycle.
...
| Ownership is not a gate | `DevSecOpsOwnership` no longer blocks ticket/Teams-alert creation for new or existing rows — severity/bucket alone decides. See [Ownership no longer gates tickets or alerts](#ownership-no-longer-gates-tickets-or-alerts) above |
...
| Lineage tracking via `NewSecretKeyId` | A rotated secret's brand-new row automatically inherits `TeamName`/`ProductName`/`ProductTeamkeyVaultName` from its parent row, so the team fills these in once per app, not once per rotation cycle |
