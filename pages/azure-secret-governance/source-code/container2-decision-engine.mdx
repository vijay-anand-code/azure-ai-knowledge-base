...
async def run_secret_monitoring(
    ...
    owner_email:            str = "",               # legacy single-email param
    fetch_app_owners:       Callable | None = None,  # legacy — not used
    get_owned_app_ids:      Callable | None = None,  # legacy — not used
) -> dict:
    ...
    # Build SP index — filtered by ownership
    ...
    # ── PHASE 1: Jira tickets for new secrets ────────────────────────────────
    ...
    # ── PHASE 2: SharePoint + Teams for new secrets ───────────────────────────
    async def _write_new_secret(res: dict):
        ...
        if bucket != "P3":
            try:
                ...
                fields["AlertStatus"] = "JiraRaised"
                return {"action": "sp_created", "teams": False,
                        "error": f"Teams alert failed: {e}"}
        else:
            fields["AlertStatus"] = "JiraRaised"
        ...
        try:
            ...
            return {"action": "sp_created", "teams": teams_sent,
                    "error": f"SP write failed: {e}"}
    ...
    # ── PHASE 4: Existing secrets ─────────────────────────────────────────────
    async def _process_existing(c: dict, existing: dict):
        try:
            return await _handle_existing_secret(
