...
| 6 | Jira ticket still open | Live status via `get_jira_issue()` | Status is `done`/`closed`/`resolved` — row instead flips to `JiraClosed-Unverified` for manual review, no rotation happens |
...
<Callout type="info">
  ...
  `create_azure_secret()`/`store_rotated_secret()` at all — it means an engineer already generated
