...
**What it does:** Runs in two phases — first checks `RotatedPendingDeployment` rows to see if Jira tickets have been closed (Phase A), then proactively rotates P1-P4 secrets whose SharePoint row has `ManualAppOwners` filled in by a human (Phase B). New secret values are stored in the rotation Key Vault and a comment is posted to the Jira ticket with retrieval instructions.
...
| `ManualAppOwners` gate | Rotation is gated on `ManualAppOwners` in SharePoint, filled in by a human — not the auto-stamped `AppOwners` column, which is informational only |
...
so also make sure that field is filled in by a human on this row. Once both are set,
rotation will act on this app on its next run, provided the app's secret is also in
a `P1`-`P4` bucket.
