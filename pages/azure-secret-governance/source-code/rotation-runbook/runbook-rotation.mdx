...
| `NewSecretKeyId` presence is a second, independent "already rotated" guard | Rotation writes `NewSecretKeyId` on the same row it flips to `RotatedPendingDeployment`. If someone manually resets `AlertStatus` back to an actionable value without also clearing `NewSecretKeyId`, this row-level check still skips it, so re-rotation now requires resetting both fields, not just the status |
...
| `ManualAppOwners` gate | Rotation is gated on `ManualAppOwners` in SharePoint, filled in by a human or resolved from `ProductName` by monitoring or owner sync — not the auto-stamped `AppOwners` column, which is informational only |
| `ProductTeamkeyVaultName` derives a URL, doesn't store one | The row's value (e.g. `kv-product-a`) has the standard `https://<name>.vault.azure.net/` suffix appended directly, rather than being looked up against a separately maintained list. Blank means the shared rotation vault. A `SecretClient` for a given derived URL is built once and reused for the rest of the run |
...
| 2 | Owned by a human | `ManualAppOwners` | Blank — must be filled by a person directly, or resolved earlier by monitoring/owner sync from `ProductName` |
...
| 5 | Not already rotated | `NewSecretKeyId` | Already has a value — this exact row was already rotated once. Re-rotating it requires clearing this field, resetting `AlertStatus` alone is no longer enough |
| 6 | Has a Jira ticket | `JiraTicketKey` | Blank — logged as an error (`"no JiraTicketKey, cannot rotate"`), never silently allowed through |
| 7 | Jira ticket still open | Live status via `get_jira_issue()` | Status is `done`/`closed`/`resolved` — row instead flips to `JiraClosed-Unverified` for manual review, no rotation happens |
| 8 | App still exists in Azure | `app_info_by_client_id` lookup | The app registration's `appId` isn't found across the scanned tenants |

If a row clears all eight, `store_rotated_secret()` resolves the target vault (`ProductTeamkeyVaultName`
if set, else the shared rotation vault) and secret name (reuse `NewSecretVaultName` if set, else
generate and write one back — see "Secret naming" below), creates a new Key Vault secret version,
posts a Jira comment with retrieval instructions, and writes `AlertStatus = RotatedPendingDeployment`
plus `NewSecretKeyId`/`NewSecretVaultName`/`RotationDetectedDate` back to the row.
...
<Callout type="warning">
  This only holds together if a row's `ProductTeamkeyVaultName` also stays stable once it's set.
...
Changes in v3 (this version):
  ...
  5. BUCKET SCHEME CORRECTED (NEW in v3) — this file previously had its
     OWN, stale bucket classifier (P4=safe/61+, P3=31-60, P2=8-30,
     P1=0-7 — the OLD pre-split scheme) that disagreed with
     decision_engine.py's current P1=0-3/P2=4-7/P3=8-30/P4=31-60/P5=61+.
     _classify_bucket() now matches exactly. ROTATION_ELIGIBLE_BUCKETS is
     {"P1","P2","P3","P4"} — same 0-60 day range as before, re-expressed
     under the current names. P5 is never rotation-eligible.
...
def store_rotated_secret(secret_name: str, secret_value: str,
    ...
    ProductTeamkeyVaultName set, uses that specific vault instead — a
...
def run_secret_rotation(rotation_threshold_days: int = DEFAULT_ROTATION_THRESHOLD) -> dict:
    ...
    for item in candidates:
        ...
        existing_new_key_id = (f.get("NewSecretKeyId") or "").strip()
        if existing_new_key_id:
            summary["rowsSkippedRotated"] += 1
            log.debug("Skipping %s — NewSecretKeyId already present (%s), row was already rotated",
                      app_id, existing_new_key_id)
            continue
        ...
        try:
            ...
            raw_vault_key = (f.get("ProductTeamkeyVaultName") or "").strip()
