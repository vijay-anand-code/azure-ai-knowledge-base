...
| `OWNER-EMAILS` (comma-separated) | Supports multi-owner environments. Falls back to legacy `OWNER-EMAIL` single-value secret for backwards compatibility |
...
| `OWNER-EMAILS` | Comma-separated owner filter (e.g. `uami-secret-monitor, admin@contoso.com`) |

<Callout type="warning">
  The runbook will only rotate secrets for apps where `AppOwners` in SharePoint contains
  at least one value from `OWNER-EMAILS`. Apps where the UAMI is not an Entra owner
  return HTTP 403 from the Graph `addPassword` endpoint — a second layer of enforcement.
</Callout>
...
Changes vs previous version:
  - Added "Discovered" to ACTIONABLE_STATUSES — rows stamped by discovery
    runbook but not yet processed by monitoring are now eligible for rotation
    if rotation is triggered manually before monitoring runs.
  - OWNER-EMAIL → OWNER-EMAILS (comma-separated) for multi-owner support.
    Phase B filters by any-of-list match against AppOwners column.
    Falls back to legacy OWNER-EMAIL secret if OWNER-EMAILS not found.
